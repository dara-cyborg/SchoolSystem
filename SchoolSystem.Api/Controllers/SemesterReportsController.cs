...
public class SemesterReportsController : ControllerBase
{
    ...
    /// <summary>
    /// Get a semester report by class, semester, and school year
    /// [homeroom]
    /// </summary>
    [HttpGet("by-class")]
    [Authorize(Policy = "HomeroomOnly")]
    public async Task<IActionResult> GetSemesterReportByClass(
        [FromQuery] int classId,
        [FromQuery] short semester,
        [FromQuery] short schoolYear,
        CancellationToken cancellationToken = default)
    {

        var result = await _semesterReportService.GetReportByClassAsync(classId, semester, schoolYear, cancellationToken);

        if (result == null)
            return NotFound(new { message = "No report found for the specified class, semester, and school year." });
